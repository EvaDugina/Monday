...
- Старт frontend:
  ...
  - если сервер пустой, но локальный snapshot отличается от дефолтного, клиент seed-ит сервер локальным состоянием
...
- `src/App.tsx`
  - orchestration UI, category state, sync loop, backup loop, archive flow, conflict flow, local background decoration flow
- `src/api.ts`
