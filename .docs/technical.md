...
- Управление погодой (`WeatherControls`, `src/weatherControls.ts`, localStorage `monday:weather-controls`, не синхронизируется):
  - виджет несёт тумблеры дождь/небо/облака + gear (открывает `WeatherControlModal`); модалка задаёт интенсивность дождя (авто/ручную), насыщенность неба, прозрачность/parallax/скорость облаков и режим редактирования
  - параметры облаков/неба применяются как CSS-переменные на `<html>` (`--cloud-opacity`, `--cloud-speed`, `--sky-strength`, `--cloud-{a,b,c}-{x,y}`, `--parallax-*`) — так React не конфликтует с императивной записью; насыщенность неба = alpha `.app[data-sky]`
  - режим редактирования включает `pointer-events` у облаков и перетаскивание (offset на облако), на время редактирования дрейф и parallax отключаются
